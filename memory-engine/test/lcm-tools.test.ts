...
describe("LCM tools session scoping", () => {
  ...
  it("cc_expand query mode infers conversationId from delegated grant", async () => {
    const retrieval = {
      grep: vi.fn(async () => ({
        messages: [],
        summaries: [
  ...
  it("cc_grep forwards since/before and includes local timestamps in text output", async () => {
  ...
  it("cc_describe blocks cross-conversation lookup unless allConversations=true", async () => {
    const retrieval = {
      grep: vi.fn(),
      expand: vi.fn(),
    ...
    expect((scoped.details as { error?: string }).error).toContain("Not found in conversation scope");
