...
describe("RSMA Relations Integration: extraction → relation pipeline", () => {
  ...
  it("'Cassidy works for Sam' creates a relation", () => {
    ...
    const relations = db.prepare("SELECT * FROM entity_relations").all() as Array<Record<string, unknown>>;
    expect(relations.length).toBe(1);
    expect(relations[0].predicate).toBe("works_for");
  });
  ...
  it("'Project uses PostgreSQL' creates a relation (predicate != 'is')", () => {
    ...
    const relations = db.prepare("SELECT * FROM entity_relations").all() as Array<Record<string, unknown>>;
    expect(relations.length).toBe(1);
    expect(relations[0].predicate).toBe("uses");
  });
