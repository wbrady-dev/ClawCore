...
describe("LcmContextEngine.assemble canonical path", () => {
  ...
  it("adds recall workflow guidance when summaries are present", async () => {
    ...
    expect(promptAddition).toContain("3. `cc_recall`");
    // Usage patterns
    expect(promptAddition).toContain("cc_recall(summaryIds:");
    expect(promptAddition).toContain("cc_recall(query:");
  ...
  it("emphasizes expand-before-asserting when summaries are deeply compacted", async () => {
    ...
    // Full recall flow
    expect(promptAddition).toContain("1) `cc_grep` to locate relevant summary/message IDs");
    expect(promptAddition).toContain("2) `cc_recall` with a focused prompt");
