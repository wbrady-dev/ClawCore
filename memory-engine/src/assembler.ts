...
function buildSystemPromptAddition(summarySignals: SummaryPromptSignal[]): string | undefined {
  ...
  sections.push(
    ...
    "",
    "**Recall priority:** ThreadClaw memory tools first (cc_grep, cc_describe, cc_recall), then qmd (for Granola/Limitless/pre-ThreadClaw data).",
    "",
    "**Tool escalation:**",
    "1. `cc_grep` — search by regex or full-text across messages and summaries",
    "2. `cc_describe` — inspect a specific summary (cheap, no sub-agent)",
    "3. `cc_recall` — deep recall: spawns bounded sub-agent, expands DAG, returns answer with cited summary IDs (~120s, don't ration it)",
    "",
    "**`cc_recall` usage** — two patterns (always requires `prompt`):",
    "- With IDs: `cc_recall(summaryIds: [\"sum_xxx\"], prompt: \"What config changes were discussed?\")`",
    "- With search: `cc_recall(query: \"database migration\", prompt: \"What strategy was decided?\")`",
    "- Optional: `maxTokens` (default 2000), `conversationId`, `allConversations: true`",
    "",
    "**Summaries include \"Expand for details about:\" footers** listing compressed specifics. Use `cc_recall` with that summary's ID to retrieve them.",
  );
  ...
  if (heavilyCompacted) {
    sections.push(
      ...
      "1) `cc_grep` to locate relevant summary/message IDs",
      "2) `cc_recall` with a focused prompt",
