...
export class CompactionEngine {
  ...
  constructor(
    ...
    graphDb?: GraphDb,
    opts?: { claimExtractionEnabled?: boolean },
  ) {
    this.graphDb = graphDb ?? null;
    this.relationsEnabled = graphDb != null;
    this.claimExtractionEnabled = opts?.claimExtractionEnabled ?? false;
  }
  ...
  private async leafPass(
    ...
    // Claims: extract structured claims from compacted messages (atomic)
    if (this.graphDb && this.claimExtractionEnabled && messageContents.length > 0) {
      try {
        const graphDb = this.graphDb;
        withWriteTransaction(graphDb, () => {
          for (const msg of messageContents) {
            const claimResults = extractClaimsFast(msg.content, {
              sourceType: "message",
              sourceId: String(msg.messageId),
            });
