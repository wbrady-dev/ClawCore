...
export async function createLcmSummarizeFromLegacyParams(params: {
  ...
  for (const level of summaryLevels) {
    ...
    params.deps.log.warn(
      `[cc-mem] summaryModel "${level.model}" at "${level.levelName}" has no summaryProvider or provider prefix. Will attempt resolution without provider.`
    );
    resolvedSummary = { model: level.model, provider: undefined };
    break;
  }
  ...
  try {
    ...
    console.error(`[cc-mem] createLcmSummarize: resolveModel FAILED:`, err instanceof Error ? err.message : err);
    return undefined;
  }

  const { provider, model } = resolved;
  if (!provider || !model) {
    console.error(`[cc-mem] createLcmSummarize: empty provider="${provider}" or model="${model}"`);
    return undefined;
  }
  const authProfileId =
  ...
  return async (
    ...
    try {
      result = await withTimeout(
        ...
        LLM_TIMEOUT_MS,
        `summarization timed out after ${LLM_TIMEOUT_MS / 1000}s`,
      );
    } catch (err) {
      // Timeout or LLM failure — fall back to deterministic truncation.
      // Do NOT propagate: that would bypass the escalation chain entirely.
      console.error(
        `[cc-mem] summarization call failed; provider=${provider}; model=${model}; error=${
    ...
    if (!summary) {
      ...
      if (envelopeNormalized.summary) {
        ...
        console.error(
          `[cc-mem] recovered summary from response envelope; provider=${provider}; model=${model}; ` +
    ...
    if (!summary) {
      ...
      const diagParts = [
        `[cc-mem] empty normalized summary on first attempt`,
        `provider=${provider}`,
        `model=${model}`,
      ...
      try {
        const retryResult = await withTimeout(params.deps.complete({
          ...
          temperature: 0.05,
          reasoning: "low",
        }), LLM_TIMEOUT_MS, "summarization retry timed out");
        ...
        if (summary) {
          summarySource = "retry";
          console.error(
            `[cc-mem] retry succeeded; provider=${provider}; model=${model}; ` +
          ...
          const retryParts = [
            `[cc-mem] retry also returned empty summary`,
            `provider=${provider}`,
            `model=${model}`,
        ...
        console.error(
          `[cc-mem] retry failed; provider=${provider} model=${model}; error=${
    ...
    if (!summary) {
      ...
      console.error(
        `[cc-mem] all extraction attempts exhausted; provider=${provider}; model=${model}; source=fallback`,
      );
      return buildDeterministicFallbackSummary(text, targetTokens);
    }
    ...
    if (summarySource !== "content") {
      console.error(
        `[cc-mem] summary resolved via non-content path; provider=${provider}; model=${model}; source=${summarySource}`,
      );
    }
