...
export class IntegrityChecker {
  ...
  private async checkContextItemsOrdered(
    ...
    // Verify strictly monotonically increasing ordinals (gaps are allowed)
    const violations: { index: number; prev: number; current: number }[] = [];
    for (let i = 1; i < items.length; i++) {
      if (items[i].ordinal <= items[i - 1].ordinal) {
        violations.push({
          index: i,
          prev: items[i - 1].ordinal,
          current: items[i].ordinal,
        });
      }
    }
    ...
    if (violations.length === 0) {
      return {
        name: "context_items_ordered",
        status: "pass",
        message: `All ${items.length} context items have strictly increasing ordinals`,
      };
    }
    ...
    return {
      name: "context_items_ordered",
      status: "fail",
      message: `Found ${violations.length} ordinal ordering violation(s) in context items`,
      details: { violations },
    };
  }
