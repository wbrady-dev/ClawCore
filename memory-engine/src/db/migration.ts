...
/**
 * Extract agent_id from a session_id string.
 *
 * Session keys may follow the `agent:<agentId>:<suffix>` convention.
 * Returns the extracted agentId, or "main" for non-agent session keys.
 */
function extractAgentIdFromSessionId(sessionId: string): string {
...
function backfillConversationAgentIds(db: DatabaseSync): void {
  const rows = db
    .prepare(`SELECT conversation_id, session_id FROM conversations WHERE agent_id IS NULL`)
    .all() as Array<{ conversation_id: number; session_id: string }>;
  if (rows.length === 0) {
    return;
  }
...
export function runLcmMigrations(
  ...
  if (!hasAgentId) {
    ...
    // Backfill: parse session_id to extract agent_id for existing rows.
