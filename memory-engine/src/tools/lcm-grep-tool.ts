...
const LcmGrepSchema = Type.Object({
  ...
  allConversations: Type.Optional(
    Type.Boolean({
      description:
        "Set true to search across all conversations for this agent. Ignored when conversationId is provided.",
    }),
  ),
  crossAgent: Type.Optional(
    Type.Boolean({
      description:
        "When combined with allConversations=true, searches across all agents (not just the current agent). Use sparingly.",
    }),
  ),
  since: Type.Optional(
    Type.String({
