...
function buildExpansionRecursionRecoveryGuidance(originSessionKey: string): string {
  return (
    "Recovery: In delegated sub-agent sessions, call `cc_expand` directly and synthesize " +
    "your answer from that result. Do NOT call `cc_recall` from delegated context. " +
    `If deeper delegation is required, return to the origin session (${originSessionKey}) ` +
    "and call `cc_recall` there."
  );
}
