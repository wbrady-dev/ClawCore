...
const LcmDescribeSchema = Type.Object({
  ...
  allConversations: Type.Optional(
    Type.Boolean({
      description:
        "Set true to allow lookups across all conversations for this agent. Ignored when conversationId is provided.",
    }),
  ),
  crossAgent: Type.Optional(
    Type.Boolean({
      description:
        "When combined with allConversations=true, allows lookups across all agents. Use sparingly.",
    }),
  ),
...
export function createLcmDescribeTool(input: {
  ...
  return {
    ...
    async execute(_toolCallId, params) {
      ...
      if (conversationScope.conversationId != null || conversationScope.conversationIds) {
        const itemConversationId =
          result.type === "summary" ? result.summary?.conversationId : result.file?.conversationId;
        if (itemConversationId != null) {
          const allowed = conversationScope.conversationId != null
            ? itemConversationId === conversationScope.conversationId
            : conversationScope.conversationIds
              ? conversationScope.conversationIds.includes(itemConversationId)
              : true;
          if (!allowed) {
            return jsonResult({
              error: `Not found in conversation scope: ${id}`,
      ...
      return jsonResult(result);
        ...
        return jsonResult({ error: `cc_describe failed: ${err instanceof Error ? err.message : String(err)}` });
      }
    },
  };
}
