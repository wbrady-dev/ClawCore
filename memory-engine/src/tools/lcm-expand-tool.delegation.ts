...
function buildDelegatedExpansionTask(params: {
  ...
  return [
    ...
    "",
    "Rules:",
    "- In delegated context, use `cc_expand` directly for retrieval.",
    "- DO NOT call `cc_recall` from this delegated session.",
    "- Keep summary concise and factual.",
    "- Synthesize findings from the `cc_expand` result before returning.",
    ...
    const response = (await params.deps.callGateway({
      ...
      params: {
        ...
        extraSystemPrompt: params.deps.buildSubagentSystemPrompt({
          depth: 1,
          maxDepth: 8,
          taskSummary: "Run cc_expand and return JSON findings",
        }),
      },
      timeoutMs: 10_000,
    })) as { runId?: string };
