...
export type MessageSearchInput = {
  conversationId?: ConversationId;
  /** When set, restrict search to these conversation IDs (agent-scoped allConversations). */
...
export class ConversationStore {
  ...
  async createConversation(input: CreateConversationInput): Promise<ConversationRecord> {
    const result = this.db
      ...
      .run(input.sessionId, input.agentId ?? null, input.title ?? null);
  ...
  async getOrCreateConversation(
    sessionId: string,
    options?: { title?: string; agentId?: string },
  ...
  async getConversationIdsByAgentId(agentId: string): Promise<ConversationId[]> {
    const rows = this.db
      .prepare(`SELECT conversation_id FROM conversations WHERE agent_id = ?`)
      .all(agentId) as unknown as Array<{ conversation_id: number }>;
  ...
  async searchMessages(input: MessageSearchInput): Promise<MessageSearchResult[]> {
    ...
    const cIds = input.conversationIds;

    if (input.mode === "full_text") {
      if (this.fts5Available) {
        try {
          // Try strict AND first
          const strict = this.searchFullText(
            input.query,
            limit,
            input.conversationId,
            input.since,
      ...
      return this.searchLike(input.query, limit, input.conversationId, input.since, input.before, cIds);
    }
    return this.searchRegex(input.query, limit, input.conversationId, input.since, input.before, cIds);
  }
  ...
  private searchFullText(
    ...
    where.push(...convFilter.where);
    args.push(...convFilter.args);
    if (since) {
  ...
  private searchRegex(
    ...
    const args: Array<string | number> = [...convFilter.args];
    if (since) {
      where.push("julianday(created_at) >= julianday(?)");
      args.push(since.toISOString());
    }
