...
function HomeScreen({ onAction }: { onAction: (action: string) => void }) {
  ...
  const items: MenuItem[] = [
    ...
    { label: "Sources", value: "sources", description: sourceIcons.length > 0 ? sourceIcons.map((s: string) => s.replace(/[●○]\s*/g, "")).join(", ") : undefined },
  ...
  return (
    <Box flexDirection="column">
      ...
      {!anyRunning && sourceIcons.length === 0 && docCount === 0 && (
        <Text>{t.warn("  \u2192 Get started: select 'Start Services' to begin, then add folders from 'Configure' or connect cloud sources from 'Sources'")}</Text>
      )}
